...
class Elysia {
    ...
    mount(path, handle) {
        if (typeof path === 'function' || path.length === 0 || path === '/') {
            ...
            const handler = async ({ request, path }) => run(new Request((0, utils_1.getHostname)(request.url) + path || '/', request));
            this.all('/', handler, {
                type: 'none'
            });
            this.all('/*', handler, {
        ...
        const handler = async ({ request, path }) => handle(new Request((0, utils_1.getHostname)(request.url) + path.slice(length) || '/', request));
